...
You can run a demo file [here](https://github.com/rjake/whereiation/blob/master/inst/demo_analysis.Rmd). At the bottom are a lot of `df <-...; dep_var <-...; ignore_cols <- ...;` for examples that can be run within R. You will have to install some of the packages in order to use them. For example, the `AER::Fatalities` data set is `Fatalities` out of the `AER` package. To see what is on your machine, you can use the top of [this script](https://github.com/rjake/whereiation/blob/master/inst/find_datasets.R).
